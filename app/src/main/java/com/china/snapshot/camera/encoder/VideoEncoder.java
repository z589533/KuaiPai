...
import com.china.snapshot.camera.glutils.RenderThread;
...
public class VideoEncoder extends MediaEncoder {
    ...
    private RenderThread mRenderHandler;
    private Surface mSurface;

    public VideoEncoder(final MuxerWrapper muxer, final MediaEncoderListener listener, final int width, final int height) {
        super(muxer, listener);
        if (DEBUG) Log.i(TAG, "VideoEncoder: ");
        mWidth = width;
        mHeight = height;
        mRenderHandler = RenderThread.createHandler(TAG);
    }

    public void setFlag(int flag) {
        if (mRenderHandler != null) {
            mRenderHandler.setFlag(flag);
        }
    }
